...
Class PathFinding_AStar
  ...
  Protected Overrides Function OnUserUpdate(elapsedTime As Single) As Boolean
    ...
    Dim selectedNodeX = GetMouseX \ nodeSize
    Dim selectedNodeY = GetMouseY \ nodeSize

    If GetMouse(0).Released Then ' Use mouse to draw maze, shift and ctrl to place start and end
      If selectedNodeX >= 0 AndAlso selectedNodeX < m_mapWidth Then
        If selectedNodeY >= 0 AndAlso selectedNodeY < m_mapHeight Then

          If GetKey(Key.SHIFT).Held Then
            m_nodeStart = m_nodes(selectedNodeY * m_mapWidth + selectedNodeX)
          ElseIf GetKey(Key.CTRL).Held Then
    ...
    For x = 0 To m_mapWidth - 1
      For y = 0 To m_mapHeight - 1
        For Each n In m_nodes(y * m_mapWidth + x).Neighbours
          DrawLine(x * nodeSize + nodeSize \ 2, y * nodeSize + nodeSize \ 2, n.X * nodeSize + nodeSize \ 2, n.Y * nodeSize + nodeSize \ 2, Presets.DarkBlue)
        Next
      Next
    Next
