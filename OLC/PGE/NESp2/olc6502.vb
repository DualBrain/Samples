...
#End If
  ...
  Public Function Disassemble(nStart As UShort, nStop As UShort) As Dictionary(Of UShort, String)
    ...
    Dim hex = Function(n As UInteger, d As Byte)
                ...
                'Dim s = New StringBuilder(d, d)
                'For i = d - 1 To 0 Step -1
                '  s(i) = "0123456789ABCDEF"(CInt(n) And &HF)
                '  n >>= 4
                'Next
                'Return s.ToString()
              End Function
    ...
    While addr < nStop
      ...
      Dim sInst As String = "$" + hex(addr, 4) + ": "

      ' Read instruction, and get its readable name
      Dim opcode As Byte = bus.Read(CUShort(Fix(addr)), True)
      addr += 1US
      ...
      ElseIf lookup(opcode).addrmode?.Method.Name = NameOf(IMM) Then
        value = bus.Read(CUShort(Fix(addr)), True)
        addr += 1US
        sInst += "#$" + hex(value, 2) + " {IMM}"
      ElseIf lookup(opcode).addrmode?.Method.Name = NameOf(ZP0) Then
        lo = bus.Read(CUShort(Fix(addr)), True)
        addr += 1US
        hi = &H0
        sInst += "$" + hex(lo, 2) + " {ZP0}"
      ...
      ElseIf lookup(opcode).addrmode?.Method.Name = NameOf(ABS) Then
        lo = bus.Read(CUShort(Fix(addr)), True)
        addr += 1US
        hi = bus.Read(CUShort(Fix(addr)), True)
        addr += 1US
