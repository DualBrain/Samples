...
' Converted to olcPixelGameEngineVB

Option Explicit On
Option Strict On
Option Infer On

Imports Olc.PIXEL_TYPE
Imports Olc.COLOUR
Imports Olc
...
#End Region
  ...
  Protected Overloads Function Draw(x As Single, y As Single) As Boolean
    Return Draw(CInt(Fix(x)), CInt(Fix(y)))
  End Function

  Protected Overrides Function Draw(x As Integer, y As Integer) As Boolean ', Optional c As Integer = 9608, Optional col As COLOUR = 15)
    Dim fx, fy As Single
    WrapCoordinates(x, y, fx, fy)
    x = CInt(fx) : y = CInt(fy)
    Return MyBase.Draw(x, y) ', c, col)
  End Function

  Private Sub DrawWireFrameModel(vecModelCoordinates As List(Of (X As Single, Y As Single)), x As Single, y As Single, Optional r As Single = 0.0F, Optional s As Single = 1.0F, Optional col As COLOUR = FG_WHITE)
